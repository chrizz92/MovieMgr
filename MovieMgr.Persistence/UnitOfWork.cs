    ...
    public class UnitOfWork : IUnitOfWork
    {
        ...
        private void ValidateEntity(object entity)
        {
            if(entity is Category cat)
            {
                try
                {
                    if (_dbContext.Categories.Count(c => c.CategoryName == cat.CategoryName) >= 1)
                    {
                ...
                {
                    throw new ValidationException(
                        new ValidationResult("Es existiert bereits eine Kategorie mit diesem Namen", new List<string>() { nameof(cat.CategoryName) }), null, null);
                }                
            }
        }
